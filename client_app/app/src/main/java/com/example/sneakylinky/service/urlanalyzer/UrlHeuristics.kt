...
data class BlockResult(
    ...
    val reasons: List<Reason>  // present concerns only (no per-test scores)
)
...
/* ──────────────────────────────────────────────────────────────────────────────
   ...
   Log: "enc: present"
   ──────────────────────────────────────────────────────────────────────────── */
private const val ENC_BOOL_SCORE = 0.40
...
suspend fun hMedNearWhitelist(canon: CanonUrl): Double {
    val host = canon.hostAscii?.lowercase() ?: return 0.0
    ...
    if (best <= 0.0 + 1e-12) {
        if (nearest != null) {
            Log.d(TAG, "med: exact=${nearest} s=0.00")
        }
        return 0.0
    }
...
fun hPhishKeywords(canon: CanonUrl): Double {
...
/* =============================================================================
   ...
   ============================================================================= */
suspend fun analyzeAndDecide(canon: CanonUrl): BlockResult {
    val present = linkedSetOf<Reason>()            // ordered, unique reasons
    val softContribs = mutableListOf<Pair<Reason, Double>>()  // only non-critical

    // --- Booleans — CRITICAL (report + hard rule) ---
    if (hIpHost(canon))              present += Reason.IP_HOST
    if (hMixedScript(canon))         present += Reason.MIXED_SCRIPT
    if (hUserInfo(canon))            present += Reason.USERINFO_PRESENT
    if (hUnfamiliarTld(canon))       present += Reason.UNFAMILIAR_TLD

    // --- Booleans — NON-CRITICAL (soft contributions) ---
    if (hPortSchemeMismatch(canon)) {
        present += Reason.PORT_SCHEME_MISMATCH
        softContribs += Reason.PORT_SCHEME_MISMATCH to 1.0  // will be weighted
    }
    if (hEncodedParts(canon)) {
        present += Reason.ENCODED_PARTS
        softContribs += Reason.ENCODED_PARTS to 1.0         // will be weighted
    }

    // --- Numeric (soft contributions if >0) ---
    hMedNearWhitelist(canon).let { s ->
        if (s > 0.0) { present += Reason.NEAR_WHITELIST_LOOKALIKE; softContribs += Reason.NEAR_WHITELIST_LOOKALIKE to s }
    }
    hUrlLength(canon).let { s ->
        if (s > 0.0) { present += Reason.LONG_URL; softContribs += Reason.LONG_URL to s }
    }
    hSubdomainDepth(canon).let { s ->
        if (s > 0.0) { present += Reason.TOO_MANY_SUBDOMAINS; softContribs += Reason.TOO_MANY_SUBDOMAINS to s }
    }
    hPhishKeywords(canon).let { s ->
        if (s > 0.0) { present += Reason.PHISH_KEYWORDS; softContribs += Reason.PHISH_KEYWORDS to s }
    }

    // Hard rule on criticals (still compute soft score for logging/telemetry)
    ...
    for ((reason, raw) in softContribs) {
        val w = when (reason) {
            Reason.PORT_SCHEME_MISMATCH -> SOFT_WEIGHTS[reason] ?: 0.0
            Reason.ENCODED_PARTS -> SOFT_WEIGHTS[reason] ?: 0.0
            Reason.NEAR_WHITELIST_LOOKALIKE -> SOFT_WEIGHTS[reason] ?: 0.0
            Reason.LONG_URL -> SOFT_WEIGHTS[reason] ?: 0.0
            Reason.TOO_MANY_SUBDOMAINS -> SOFT_WEIGHTS[reason] ?: 0.0
            Reason.PHISH_KEYWORDS -> SOFT_WEIGHTS[reason] ?: 0.0
            else -> 0.0 // criticals never enter soft score
        }
